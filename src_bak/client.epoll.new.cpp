...
CClient :: CClient( SOCKET &sckClient, struct sockaddr_in6 &sinAddress, const unsigned int &cuiTimeOut, const char &ccCompression )
{
	...
	if( epoll_ctl( gpServer->epfd_client, EPOLL_CTL_ADD, m_sckClient, &ev ) == SOCKET_ERROR )
...
bool CClient :: Update( struct epoll_event &ev_client )
{
//	if( ev_client.data.fd != m_sckClient )
//		return false;
...
//		for( int i = 0; i < gpServer->nfds_client; i++ )
		{
//			if( gpServer->events_client[i].data.fd == m_sckClient && gpServer->events_client[i].events & EPOLLIN )
			if( ev_client.events & EPOLLIN )
			{
				m_ulLast = GetTime( );
...
//				UTIL_LogPrint( "client: receive %d\n", iRecv );
				...
				else if( iRecv == SOCKET_ERROR && GetLastError( ) == EWOULDBLOCK )
				{
					return true;
				}
				else if( iRecv == 0 )
...
//		{
			...
			if( epoll_ctl( gpServer->epfd_client, EPOLL_CTL_MOD, m_sckClient, &ev ) == SOCKET_ERROR )
			{
...
//		for( int i = 0; i < gpServer->nfds_client; i++ )
		{
//			if( gpServer->events_client[i].data.fd == m_sckClient && gpServer->events_client[i].events & EPOLLOUT )
			if( ev_client.events & EPOLLOUT )
			{
				m_ulLast = GetTime( );
				...
				else if( iSend == SOCKET_ERROR && GetLastError( ) == EWOULDBLOCK )
				{
					return true;
				}
				else if( iSend == 0 )
