...
#define STR_SEPARATORS "\n\r"
	...
	m_strIPBlockFile = CFG_GetString( "cbtt_ipblock_file", "blocks.bnbt" );
	
	m_pIPBlockedList = new CAtomList( );
	
	if( !m_strIPBlockFile.empty( ) )
	{
		if( gbDebug )
			if( gucDebugLevel & DEBUG_TRACKER )
				UTIL_LogPrint( "CTracker: CBTT IP block parse list\n" );

		const string cstrIPBlockFileData( UTIL_ReadFile( m_strIPBlockFile.c_str( ) ) ); 
		const string :: size_type ciIPBlockFileDataLength( cstrIPBlockFileData.length( ) ); 

		// loop through the ban file and add shit 
		string :: size_type iStartIPBlockFileData = cstrIPBlockFileData.find_first_not_of( STR_SEPARATORS ); 
		string :: size_type iStopIPBlockFileData = 0;

		while ( iStartIPBlockFileData != string :: npos && iStartIPBlockFileData < ciIPBlockFileDataLength )
		{ 
			iStopIPBlockFileData = cstrIPBlockFileData.find_first_of( STR_SEPARATORS, iStartIPBlockFileData ); 

			if ( iStopIPBlockFileData == string :: npos || iStopIPBlockFileData > ciIPBlockFileDataLength ) 
				iStopIPBlockFileData = ciIPBlockFileDataLength; 

			m_pIPBlockedList->addItem( new CAtomString( cstrIPBlockFileData.substr( iStartIPBlockFileData, iStopIPBlockFileData - iStartIPBlockFileData ) ) ); 

			iStartIPBlockFileData = cstrIPBlockFileData.find_first_not_of( STR_SEPARATORS, iStopIPBlockFileData + 1 ); 
		}

		UTIL_LogPrint( "%s\n", gmapLANG_CFG["cbtt_ipblock_parse"].c_str( ) );
	}
}
...
//			MILLISLEEP( 10 );
	...
	nfds_client = epoll_wait( epfd_client, events_client, 128, 0 );

	for( int i = 0; i < nfds_client; i++ )
	{
		// process the clients
		for( vector<CClient *> :: iterator itClient = m_vecClients.begin( ); itClient != m_vecClients.end( ); )
		{
			if( events_client[i].data.fd == (*itClient)->m_sckClient )
			{
				if( (*itClient)->Update( events_client[i] ) )
				{
					delete *itClient;
